...
SOURCE_CONTENTS = "AAAEAAEAAAEACAABAAQudG1wAQAKAAIABnBhcmVudAEAEQAEAA1zb21lLWZpbGUudHh0AQAQAAUADHRoZSBjb250ZW50cwMABAAAAAADAAoAAwAGcGFyZW50FAAEAAAAARQABAAAAAIVAAYAAwAEAAU="
home_dir = "/home/dir"
params = []
File = {}
File.set_content = function(c)
end function
user_input = function(_)
...
blockRelHomeFunc = function(tar)
    DEBUG("Reading rel home string")
    str_idx = tar.UInt16
    text = home_dir + "/" + tar.Ascii
    tar.strings[str_idx] = text
    DEBUG(" - " + str_idx + " <- '" + text + "'")
end function
...
blockFolderFunc = function(tar)
    ...
    if out == null then
        ...
        INFO("- created foler [" + parent + "/" + name + "] (skipped - already exists)")
    end if
end function
...
blockBuildFunc = function(tar)
    ...
    src_out.copy("/tmp", "source.scr")
    res = Compile("/tmp/source.scr", "/tmp")
    if res != "" and res != null then exit("Failed building " + source_fqn + ": " + res)
    out = GetFile("/tmp/source")
...
while source_file.HasMore
    ...
    if BLOCK_HANDLERS.hasIndex(block_type) then
        DEBUG("Block handler " + block_type)
