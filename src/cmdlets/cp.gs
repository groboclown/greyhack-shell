...
Cp.Run = function(context, args, session)
    ...
    if args.Unnamed.len < 2 then
        ContextLib.Cli.ShowSummary(Cp.usage, context, context.Errors)
        return
    end if

    // The target does not need to exist as a file.
    if args.Unnamed[-1].File != null then
        target = FileLib.Paths.NormalizePath(FileLib.Paths.SplitPath(args.Unnamed[-1].File.path))
        is_folder = files[-1].File.is_folder
    else
        target = FileLib.Paths.NormalizePath(FileLib.Paths.SplitPath(args.Unnamed[-1].Value))
        is_folder = false
    end if
